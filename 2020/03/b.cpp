...
int mvRight[5] = { 1, 3, 5, 7, 1 };
int mvDown[5] = { 1, 1, 1, 1, 2 };
int lap = 0;
...
int main () {
  ...
  while (lap < sizeof(mvDown)/sizeof(mvDown[0])) {
    cout << "Lap " << lap <<", " << "Right " << mvRight[lap] << ", down " << mvDown[lap] << "." << endl;
    int encounter = 0;
    int x = 0;
    int y = 0;
    while ( x < lines) {
    ...
    multiplied *= encounter;
    lap++;
  }
  cout << "Multipled: " << multiplied << endl;
  return 0;
}
